...
\begin{qbox}
Problem Stem
\answeredbox{この数列は線形非同次漸化式です。まず、同次方程式 $b_{n+1} = 2b_n$ を考えると、一般解は $b_n = C 	imes 2^n$ です。次に、特解を考えます。定数解を仮定して $a_n = A$ とすると、$A = 2A + 1$ となり、$A = -1$。したがって、特解は $a_n = -1$。\n\n一般解は、$a_n = C 	imes 2^n - 1$。初期条件 $a_1 = 3$ を代入すると、$C 	imes 2 - 1 = 3$、よって $C = 2$。\n\nしたがって、$a_n = 2 	imes 2^n - 1 = 2^{n+1} - 1$。}
\end{qbox}
